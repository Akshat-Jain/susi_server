...
public class JSONConverter {
  ...
  public static String getDescription(List<Leg> legs) {
    ...
    for (int i = 0; i < legs.size(); i++) {
      if (!(legs.get(i) instanceof Trip.Public))
      continue;
      if (i == 0) {
        response += "Take the ";
      } else {
      ...
      response += " (departing at " + pub.getDepartureTime().getHours(

      ) + ":" + pub.getDepartureTime().getMinutes();
      if (pub.getDepartureDelay() != null) {
        response += ", +" + pub.getDepartureDelay() / 60000.0 + " mins delay";
      }
      response += ") to ";
      response += pub.arrival.name;
      response += " (arriving at " + pub.getArrivalTime().getHours(

      ) + ":" + pub.getArrivalTime().getMinutes();
      if (pub.getArrivalDelay() != null) {
