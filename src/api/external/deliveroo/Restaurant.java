...
public class Restaurant {
  ...
  public static List<Restaurant> fetchRestaurants(
    ...
    try {
      ...
      CloseableHttpResponse httpResponse = client.getHttpClient().execute(
        httpGet
      );
      if (httpResponse.getStatusLine().getStatusCode() == 200) {
        ...
        assertThat(
          contentMimeType,
          equalTo(ContentType.APPLICATION_JSON.getMimeType())
        );
  ...
  public Restaurant(JSONObject jsonObject) {
    ...
    this.coordinate = new Coordinate(
      coordinateJsonArray.getDouble(0),
      coordinateJsonArray.getDouble(1)
    );
    ...
    this.openingHours = this.parseOpeningHours(
      jsonObject.getJSONObject("opening_hours")
    );

    JSONObject targetDeliveryTimeJson = jsonObject.getJSONObject(
      "target_delivery_time"
    );
    String range = targetDeliveryTimeJson.keys().next();
    this.targetDeliveryTime = new TargetDeliveryTime(
      range,
      targetDeliveryTimeJson.getString(range)
    );
    this.menuTags = this.parseMenuTags(
      jsonObject.getJSONObject("menu").getJSONArray("menu_tags")
    );
    this.open = jsonObject.getBoolean("open");
  }

  private List<OpeningHours> parseOpeningHours(
    JSONObject openingHoursJsonObject
  ) {
  ...
  public Restaurant fetchMenuItems(Client client)
    throws
      URISyntaxException,
      ClientProtocolException,
      IOException {
    ...
    JSONArray menuItemsJsonArray = (new JSONObject(rawJson)).getJSONObject(
      "menu"
    ).getJSONArray("menu_items");
