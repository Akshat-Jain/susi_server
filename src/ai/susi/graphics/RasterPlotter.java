...
public class RasterPlotter {
  ...
  public void line(
    ...
    final int Bx,
    final int By,
    final Long colorLine,
    final int intensityLine,
    final Long colorDot,
    final int intensityDot,
    final int dotDist,
    final int dotPos,
    final int dotRadius,
    final boolean dotFilled
  ) {
    ...
    if (dX >= dY) {
      ...
      for (; dX >= 0; dX--) {
        ...
        if (dotc == dotPos) {
          ...
          if (dotRadius == 0)
          this.plot(Ax, Ay, intensityDot); else if (dotRadius > 0)
          dot(Ax, Ay, dotRadius, dotFilled, intensityDot);
        }
        dotc++;
        if (dotc == dotDist)
        dotc = 0;
  ...
  public void lineDot(
    ...
    setColor(lineColor);
    line(x0, y0, x3, y3, 100); // draw line from {x0, y0} to dot border
    setColor(dotColor);
    dot(x2, y2, radius, true, 100); // draw dot at {x2, y2}
  }
  ...
  public void arcConnect(
    ...
    final Long colorMessage,
    final int intensityMessage
  ) {
  ...
  public void insertBitmap(
    ...
    for (int i = 0; i < heightSrc; i++) {
      for (int j = 0; j < widthSrc; j++) {// pixel in legal area?
    ...
    if (filter == FilterMode.FILTER_ANTIALIASING) {
      ...
      if (transX != -1) {
        filter(
          ...
          y + bitmapHeight,
          filter,
          this.image.getRGB(transX + x, transY + y)
        );
      }
    } else {
  ...
  private void filter(
    ...
    final BufferedImage image2 = new BufferedImage(
      width2,
      height2,
    ...
    for (int i = lox; i < rux + 1; i++) {
      for (int j = loy; j < ruy + 1; j++) {
        ...
        if (filter == FilterMode.FILTER_ANTIALIASING || filter == FilterMode.FILTER_BLUR) {// taking samples from neighbouring pixel
        ...
        if (filter == FilterMode.FILTER_ANTIALIASING && border || filter == FilterMode.FILTER_BLUR) {
          ...
          numberOfNeighbours++;
          border = false;
        } else // set value of pixel to inverted value (using XOR)
        if (filter == FilterMode.FILTER_ANTIALIASING) {
          rgbR = (rgb >> 16 & 0xff);
          rgbG = (rgb >> 8 & 0xff);
          rgbB = (rgb & 0xff);
  ...
  public static void demoPaint(final RasterPlotter m) {
    ...
    PrintTool.print(m, 0, 65, 0, "Grey", -1, false, 100);
    m.line(65, 0, 65, 300, 100);
    m.setColor(RED);
    m.line(0, 90, 100, 90, 100);
    PrintTool.print(m, 0, 85, 0, "Red", -1, false, 100);
    m.line(70, 0, 70, 300, 100);
    m.setColor(GREEN);
    m.line(0, 110, 100, 110, 100);
    PrintTool.print(m, 0, 105, 0, "Green", -1, false, 100);
    m.line(75, 0, 75, 300, 100);
    m.setColor(BLUE);
    m.line(0, 130, 100, 130, 100);
    PrintTool.print(m, 0, 125, 0, "Blue", -1, false, 100);
  ...
  public BufferedImage toIndexed() {
    ...
    BufferedImage dest = new BufferedImage(
      ...
      ) < 8 ? BufferedImage.TYPE_BYTE_BINARY : BufferedImage.TYPE_BYTE_INDEXED,
      cm
    );
  ...
  public static ByteArrayOutputStream exportImage(
    ...
    try {
      ...
      return baos;
    } catch(final IOException e) {// should not happen
  ...
  public final byte[] pngEncode(final int compressionLevel) throws IOException {
    if (this.frame == null)
    return exportImage(this.getImage(), "png").toByteArray();
    final int width = image.getWidth(null);
    final int height = image.getHeight(null);

    final Deflater scrunch = new Deflater(compressionLevel);
    ByteArrayOutputStream outBytes = new ByteArrayOutputStream(1024);
    final OutputStream compBytes = new BufferedOutputStream(
  ...
  private final static int writeInt4(
    final byte[] target,
    final int n,
    final int offset
  ) {
  ...
  private final static int writeBytes(
    final byte[] target,
    final byte[] data,
    final int offset
  ) {
  ...
  // open file automatically, works only on Mac OS X
  {
    ...
    final RasterPlotter m = new RasterPlotter(
      200,
      300,
      DrawMode.MODE_SUB,
      "FFFFFF"
