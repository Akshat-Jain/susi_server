...
public class HexGridPlotter
  ...
  public void gridPrint(
    ...
    final int y,
    final int radius,
    final String messageHigh,
    final String messageLow,
    final int align,
    int intensity
  ) {
    ...
    PrintTool.print(
      this,
      projectionX(x, y) - align * radius,
      projectionY(y) - cellwidth2 >> 2 - ((align == 0) ? radius : 0),
      0,
      messageHigh,
      align,
      false,
    ...
    PrintTool.print(
      this,
      projectionX(x, y) - align * radius,
      projectionY(y) + cellwidth2 >> 2 + 4 + ((align == 0) ? radius : 0),
      0,
      messageLow,
      align,
      false,
  ...
  public void gridPrint315(
    final int x,
    final int y,
    final int radius,
    final String message315,
    int intensity
  ) {
    if (message315 != null && message315.length() > 0)
    PrintTool.print(
      this,
      projectionX(x, y) + (radius >> 1),
      projectionY(y) + cellwidth2 >> 2 + 4,
      315,
      message315,
      -1,
      false,
  ...
  public void gridLine(
    ...
    while (x1 != x0 || y1 != y0) {
      ...
      if (dotc == dotPos) {
        ...
        if (dotRadius == 0)
        this.plot(x1, y1, intensityDot); else if (dotRadius > 0)
  ...
  private static HexGridPlotter testImage0(
    int width,
    int height,
    String bgcolor,
    String gridcolor,
    ...
    final HexGridPlotter picture = new HexGridPlotter(
      width,
      height,
      bgcolor.equals("000000") ? DrawMode.MODE_ADD : DrawMode.MODE_SUB,
      bgcolor,
      18
    );
  ...
  public static void main(final String[] args) {
    ...
    for (int i = 640; i < 700; i++) {
      animation.addFrame(
        testImage0(i, 480, "000000", "555555", "33ff33").getImage(),
        10
      );
    }
    animation.show();
