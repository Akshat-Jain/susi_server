...
public class ChartPlotter
  ...
  private final Long[] colnames = new Long[] {
    0xFFFFFFl,
    0xFFFFFFl,
    0xFFFFFFl,
    0xFFFFFFl,
  ...
  private final Long[] colscale = new Long[] {
    null,
    null,
    null,
    null,
  ...
  public ChartPlotter(
    ...
    final Long foregroundColor,
    final Long lightColor,
    ...
    this.bottomborder = bottomborder; //this.name = name;
  ...
  public void declareDimension(
    ...
    final int scale,
    final int pixelperscale,
    final int offset,
    final Long colorNaming,
    final Long colorScale,
    final String name
  ) {
  ...
  public void chartDot(
    ...
    if (dotsize == 1)
    plot(this.leftborder + x, this.height - this.bottomborder - y, 100); else
    dot(
      this.leftborder + x,
      this.height - this.bottomborder - y,
      dotsize,
      true,
      100
    );
    ...
    PrintTool.print(
      this,
      this.leftborder + x + dotsize + 2 + ((anotAngle == 315) ? -9 : 0),
      this.height - this.bottomborder - y + ((anotAngle == 315) ? -3 : 0),
      anotAngle,
      anot,
      (anotAngle == 0) ? (anot.length(

      ) * 6 + x > this.width ? 1 : -1) : ((anotAngle == 315) ? 1 : 0),
      false,
      100
    );
  }
  ...
  public void chartLine(
    final int dimension_x,
    final int dimension_y,
    final float coord_x1,
    final int coord_y1,
    final float coord_x2,
    final int coord_y2
  ) {
    ...
    line(
      this.leftborder + x1,
      this.height - this.bottomborder - y1,
      this.leftborder + x2,
      this.height - this.bottomborder - y2,
      100
    );
  }
  ...
  private void drawVerticalScale(
    ...
    assert scale > 0;
    if (pixelperscale <= 0)
    return; // this would not meet the termination condition in the while loop
    ...
    while (y > this.topborder) {
      ...
      s1 = (s >= 1000000 && s % 10000 == 0) ? Integer.toString(
        s / 1000000
      ) + "M" : (s >= 1000 && s % 1000 == 0) ? Integer.toString(
        s / 1000
      ...
      PrintTool.print(
        this,
        (left) ? this.leftborder - 4 : this.width - this.rightborder + 4,
        y,
        0,
        s1,
        (left) ? 1 : -1,
    ...
    PrintTool.print(
      this,
      (left) ? Math.max(6, x - s1max * 6 - 6) : x + s1max * 6 + 9,
      this.height - this.bottomborder,
      90,
      name,
      -1,
      false,
  ...
  public static void main(final String[] args) {
    ...
    final ChartPlotter ip = new ChartPlotter(
      660,
      240,
      bg,
      fg,
      ...
      20,
      20,
      "PEER PERFORMANCE GRAPH: PAGES/MINUTE and USED MEMORY",
      ""
    );
    ip.declareDimension(
      DIMENSION_BOTTOM,
