...
public class AnimationGIF {
  ...
  public AnimationGIF(int loops) {
    ...
    Iterator<ImageWriter> writerIterator = ImageIO.getImageWritersByFormatName(
      "GIF"
    );
  ...
  public void addImage(
    ...
    throws
      IOException {
    if (this.counter == 0) {
      iiom = writer.getDefaultImageMetadata(
  ...
  private static void setMetadata(
    ...
    Node gceNode = null;
    for (int i = 0,
    len = nodeList.getLength(); i < len; i++) {
    ...
    throw new IIOInvalidTreeException(
      "Invalid image metadata, could not find " + gceNodeName + "node.",
      null,
      tree
    );
    ...
    if (transparencyColorIndex >= 0) {
      Node transparencyFlagNode = gceNode.getAttributes().getNamedItem(
        transparencyFlagNodeName
      );
      if (transparencyFlagNode == null) {
        transparencyFlagNode = tree.getOwnerDocument().createAttribute(
          transparencyFlagNodeName
        );
        gceNode.appendChild(transparencyFlagNode);
      }
      transparencyFlagNode.setNodeValue("TRUE");
      Node transparencyIndexNode = gceNode.getAttributes().getNamedItem(
        transparencyIndexNodeName
      );
      if (transparencyIndexNode == null) {
        transparencyIndexNode = tree.getOwnerDocument().createAttribute(
          transparencyIndexNodeName
        );
        gceNode.appendChild(transparencyIndexNode);
      }
      transparencyIndexNode.setNodeValue(
        Integer.valueOf(transparencyColorIndex).toString()
      );
    }
  ...
  /**
     ...
     */
  private static void setLoops(IIOMetadata metaData, int loops)
    throws
      IIOInvalidTreeException {
    Node tree = metaData.getAsTree(formatName);
    ...
    ae.setUserObject(
      new byte[] { 0x1, (byte) (loops & 0xFF), (byte) (loops >> 8 & 0xFF) }
    );
    aes.appendChild(ae);
  ...
  /**
     ...
     */
  private static RenderedImage generateTestImage(
    int width,
    int height,
    Random r,
    double angle
  ...
  public static void main(String[] args) {
    ...
    try {
      for (int i = 0; i < framescount; i++) {
        generator.addImage(
          generateTestImage(320, 160, r, i * 2 * Math.PI / framescount),
          10,
          0
        );
      }
