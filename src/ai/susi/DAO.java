...
public class DAO {
  ...
  model_watch_dir,
  susi_skill_repo,
  private_skill_watch_dir,
  susi_private_skill_repo,
  usi_skill_repo,
  deleted_skill_dir;
  ...
  static {
    PatternLayout layout = new PatternLayout(
      "%d{yyyy-MM-dd HH:mm:ss.SSS} %p %c %x - %m%n"
    );
  ...
  public static void init(Map<String, String> configMap, Path dataPath)
    ...
    if (!susi_memory_dir_new.exists())
    susi_memory_dir_new.mkdirs();
    ...
    if (!susi_chatlog_dir.exists())
    susi_chatlog_dir.mkdirs();
    if (!susi_skilllog_dir.exists())
    susi_skilllog_dir.mkdirs();
    // TODO:
    deleted_skill_dir = new File(
      new File(DAO.data_dir, "deleted_skill_dir"),
      "models"
    );
    ...
    model_watch_dir = new File(
      new File(data_dir.getParentFile().getParentFile(), "susi_skill_data"),
      "models"
    );
    private_skill_watch_dir = new File(
      new File(
        data_dir.getParentFile().getParentFile(),
        "susi_private_skill_data"
      ),
      "users"
    );
    susi_skill_repo = new File(
      data_dir.getParentFile().getParentFile(),
      "susi_skill_data/.git"
    );
    susi_private_skill_repo = new File(
      data_dir.getParentFile().getParentFile(),
      "susi_private_skill_data/.git"
    );
    File susi_generic_skills = new File(data_dir, "generic_skills");
    if (!susi_generic_skills.exists())
    susi_generic_skills.mkdirs();
    File susi_generic_skills_media_discovery = new File(
      susi_generic_skills,
      "media_discovery"
    );
    if (!susi_generic_skills_media_discovery.exists())
    susi_generic_skills_media_discovery.mkdirs();
    // wake up susi
    File system_skills_general = new File(
      new File(conf_dir, "system_skills"),
      "general"
    );
    File system_skills_localmode = new File(
      new File(conf_dir, "system_skills"),
      "localmode"
    );
    susi = new SusiMind(
      susi_chatlog_dir,
      susi_skilllog_dir,
      system_skills_general
    );
    if (model_watch_dir.exists())
    susi.addWatchpath(new File(model_watch_dir, "general"));
    ...
    public_settings = new Settings(
      new File("data/settings/public.settings.json")
    );
    ...
    if (!private_settings.loadPrivateKey() || !public_settings.loadPublicKey(
      ...
      log(
        "Key creation finished. Peer hash: " + public_settings.getPeerHashAlgorithm(

        ) + " " + public_settings.getPeerHash()
      );
    } else {
      log(
        "Key pair loaded from file. Peer hash: " + public_settings.getPeerHashAlgorithm(
    ...
    Path authentication_path_vol = settings_dir.resolve(
      "authentication_session.json"
    );
    authentication = new JsonTray(
      authentication_path_per.toFile(),
      authentication_path_vol.toFile(),
      1000000
    );
    ...
    Path authorization_path_vol = settings_dir.resolve(
      "authorization_session.json"
    );
    authorization = new JsonTray(
      authorization_path_per.toFile(),
      authorization_path_vol.toFile(),
      1000000
    );
    ...
    Path passwordreset_path_vol = settings_dir.resolve(
      "passwordreset_session.json"
    );
    passwordreset = new JsonTray(
      passwordreset_path_per.toFile(),
      passwordreset_path_vol.toFile(),
      1000000
    );
    ...
    accounting = new JsonTray(
      accounting_path_per.toFile(),
      accounting_path_vol.toFile(),
      1000000
    );
    ...
    Path skillRating_vol = susi_skill_rating_dir.resolve(
      "skillRating_session.json"
    );
    skillRating = new JsonTray(
      skillRating_per.toFile(),
      skillRating_vol.toFile(),
      1000000
    );
    ...
    chatbot = new JsonTray(
      susiChatbot_per.toFile(),
      susiChatbot_vol.toFile(),
      1000000
    );
    ...
    Path profileDetails_per = susi_profile_details_dir.resolve(
      "profileDetails.json"
    );
    Path profileDetails_vol = susi_profile_details_dir.resolve(
      "profileDetails_session.json"
    );
    profileDetails = new JsonTray(
      profileDetails_per.toFile(),
      profileDetails_vol.toFile(),
      1000000
    );
    ...
    Path fiveStarSkillRating_per = susi_skill_rating_dir.resolve(
      "fiveStarSkillRating.json"
    );
    Path fiveStarSkillRating_vol = susi_skill_rating_dir.resolve(
      "fiveStarSkillRating_session.json"
    );
    fiveStarSkillRating = new JsonTray(
      fiveStarSkillRating_per.toFile(),
      fiveStarSkillRating_vol.toFile(),
      1000000
    );
    ...
    Path feedbackSkill_per = susi_skill_rating_dir.resolve(
      "feedbackSkill.json"
    );
    Path feedbackSkill_vol = susi_skill_rating_dir.resolve(
      "feedbackSkill_session.json"
    );
    feedbackSkill = new JsonTray(
      feedbackSkill_per.toFile(),
      feedbackSkill_vol.toFile(),
      1000000
    );
    ...
    Path feedbackLogs_vol = susi_skill_rating_dir.resolve(
      "feedbackLogs_session.json"
    );
    feedbackLogs = new JsonTray(
      feedbackLogs_per.toFile(),
      feedbackLogs_vol.toFile(),
      1000000
    );
    ...
    Path bookmarkSkill_per = susi_skill_rating_dir.resolve(
      "bookmarkSkill.json"
    );
    Path bookmarkSkill_vol = susi_skill_rating_dir.resolve(
      "bookmarkSkill_session.json"
    );
    bookmarkSkill = new JsonTray(
      bookmarkSkill_per.toFile(),
      bookmarkSkill_vol.toFile(),
      1000000
    );
    ...
    Path countryWiseSkillUsage_per = country_wise_skill_usage_dir.resolve(
      "countryWiseSkillUsage.json"
    );
    Path countryWiseSkillUsage_vol = country_wise_skill_usage_dir.resolve(
      "countryWiseSkillUsage_session.json"
    );
    countryWiseSkillUsage = new JsonTray(
      countryWiseSkillUsage_per.toFile(),
      countryWiseSkillUsage_vol.toFile(),
      1000000
    );
    ...
    Path skillUsage_vol = susi_skill_usage_dir.resolve(
      "skillUsage_session.json"
    );
    skillUsage = new JsonTray(
      skillUsage_per.toFile(),
      skillUsage_vol.toFile(),
      1000000
    );
    ...
    Path deviceWiseSkillUsage_per = device_wise_skill_usage_dir.resolve(
      "deviceWiseSkillUsage.json"
    );
    Path deviceWiseSkillUsage_vol = device_wise_skill_usage_dir.resolve(
      "deviceWiseSkillUsage_session.json"
    );
    deviceWiseSkillUsage = new JsonTray(
      deviceWiseSkillUsage_per.toFile(),
      deviceWiseSkillUsage_vol.toFile(),
      1000000
    );
    ...
    OS.protectPath(index_dir);
    // no other permissions to this path
  ...
  public static File getAssetFile(
    ...
    File storage_path = new File(
      new File(new File(assets, letter0), letter1),
      screen_name
    );
  ...
  public static String[] getConfig(
    ...
    String value = config.get(key);
    return value == null || value.length() == 0 ? default_val : value.split(
      delim
    );
  }
  ...
  public static boolean getConfig(String key, boolean default_val) {
    String value = config.get(key);
    return value == null ? default_val : value.equals("true") || value.equals(
      "on"
    ) || value.equals("1");
  }
  ...
  /**
     ...
     * @param key
     */
  public static void registerKey(ClientIdentity id, PublicKey key)
    throws
      APIException {
    JSONObject user_obj;
    try {
  ...
  public static boolean hasAccounting(
    @Nonnull
    ClientIdentity credential
  ) {
  ...
  public static Repository getRepository() throws IOException {
    ...
    if (repoFile.exists()) {
      ...
      repo = new FileRepositoryBuilder().setGitDir(
        susi_skill_repo
      ...
      repo = new FileRepositoryBuilder().setGitDir(susi_skill_repo).build();
      repo.create();
    }
    return repo;
  }
      ...
      if (mergeResult != null && mergeResult.getConflicts() != null) {
        ...
        try {
          EmailHandler.sendEmail(
            getConfig("skill_repo.admin_email", ""),
            "SUSI Skill Data Conflicts",
            getConflictsMailContent(mergeResult)
          );
        } catch(Throwable e) {
          e.printStackTrace();
        }
        ...
        push.setCredentialsProvider(
          new UsernamePasswordCredentialsProvider(
            getConfig("github.username", ""),
            getConfig("github.password", "")
          )
        );
        push.call();
      }
  ...
  public static void pushCommit(
    ...
    throws
      IOException {// fix bad email setting
    ...
    try {
      git.commit().setAllowEmpty(false).setAll(true).setAuthor(
        new PersonIdent(userEmail, userEmail)
  ...
  private static String getConflictsMailContent(MergeResult mergeResult)
    ...
    try {
      result = IO.readFileCached(
        Paths.get(DAO.conf_dir + "/templates/conflicts-mail.txt")
      );
      for (String path : mergeResult.getConflicts().keySet()) {
        ...
        for (int i = 0; i < c.length; ++i) {
          ...
          for (int j = 0; j < (c[i].length) - 1; ++j) {
            if (c[i][j] >= 0)
            conflictLines += "    Chunk for " + mergeResult.getMergedCommits(

            )[j] + " starts on line #" + c[i][j] + "\n";
          }
        }
      }
