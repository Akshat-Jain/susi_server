...
public class SusiMemory {
  ...
  public SusiMemory(File chatlog, File skilllog, int attention) {
    ...
    if (this.skilllog != null)
    try {
      FileUtils.cleanDirectory(this.skilllog);
    } catch(IOException e) {}
    // do this only as long as we are in a migration phase
  ...
  public void initializeMemory() {// initialize the unanswered list.

    if (this.chatlog != null)
    for (String identity : this.chatlog.list()) {
      // we iterate over all users and check all their conversations
      getCognitions(identity).forEach(
        cognition -> //System.out.println("** DEBUG user " + c + "; q = " + query + "; a = " + answer);
        {
  ...
  public List<TokenMapList> unanswered2tokenizedstats() {
    ...
    this.unanswered.forEach(
      (term, counter) -> {
        SusiLinguistics.tokenizeSentence(null, term).forEach(
          token -> {
            if (token.original.length() > 1)
            MapTools.incCounter(tokenCounter, token.original, counter.get());
          }
        );
      }
    );
    ...
    tokenCounter.forEach(
      (token, tokencount) -> {
        ...
        this.unanswered.forEach(
          (query, querycount) -> {//String q = query.toLowerCase();
        ...
        TokenMapList tokenmaplist = new TokenMapList(
          token,
          MapTools.deatomize(unansweredPartList),
          c.get()
        );
    ...
    LinkedHashMap<TokenMapList, Integer> sorter = MapTools.sortByValue(
      aggregator
    );
    sorter.forEach(
      (tokenmaplist, counter) -> {
  ...
  public static class TokenMapList {
    ...
    public boolean equals(Object o) {
      return o instanceof TokenMapList && ((TokenMapList) o).token.equals(
        this.token
      );
    }
  ...
  public void removeUnanswered(Pattern p) {
    ...
    if (removed)
    System.out.println("** removed unanswered pattern " + p.pattern());
  }
  ...
  public List<SusiCognition> getCognitions(String client) {
    ...
    if (identity == null) {
      if (this.chatlog == null)
      return new ArrayList<SusiCognition>();
  ...
  public SusiMemory addCognition(String client, SusiCognition cognition)
    throws
      IOException {
  ...
  public Set<String> getIntentsetNames(String client) {
    ...
    if (intentsets == null && chatlog != null) {
      ...
      if (s.endsWith(".json"))
      intents.add(s.substring(0, s.length() - 5));
      return intents;
    }
