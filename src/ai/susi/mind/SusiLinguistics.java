...
public class SusiLinguistics {
  private final static Map<
    SusiLanguage,
    Map<String, String>
  > synonyms = new ConcurrentHashMap<>(); // a map from a language to a map from a synonym to a canonical expression
  private final static Map<
    SusiLanguage,
    Map<String, String>
  > categories = new ConcurrentHashMap<>(); // a map from a language to a map from an expression to an associated category name
  private final static Map<
    SusiLanguage,
    Set<String>
  > filler = new ConcurrentHashMap<>();
  // a map from a language to a set of words that can be ignored completely

  public static void learn(SusiLanguage language, JSONObject json) {
    // initialize temporary json objects
    JSONObject syn = json.has("synonyms") ? json.getJSONObject(
      "synonyms"
    ) : new JSONObject();
    JSONArray fill = json.has("filler") ? json.getJSONArray(
      "filler"
    ) : new JSONArray();
    JSONObject cat = json.has("categories") ? json.getJSONObject(
      "categories"
    ) : new JSONObject();
    // check if synonyms exist

    if (!synonyms.containsKey(language))
    synonyms.put(language, new ConcurrentHashMap<>());
    ...
    for (String canonical : syn.keySet()) {
      ...
      a.forEach(
        synonym -> synmap.put(((String) synonym).toLowerCase(), canonical)
      );
    }
    // check if filler exist

    if (!filler.containsKey(language))
    filler.put(language, new HashSet<>());
    ...
    categories.put(language, new ConcurrentHashMap<>());
    ...
    if (cat.length() > 0) {
      for (String canonical : cat.keySet()) {
        ...
        a.forEach(
          synonym -> catforlang.put(((String) synonym).toLowerCase(), canonical)
        );
      }
    }
  }
  ...
  public static class Token {
    ...
    public String toString() {
      return "{" + "\"original\"=\"" + original + "\"," + "\"canonical\"=\"" + canonical + "\"," + "\"categorized\"=\"" + categorized + "\"" + "}";
    }
  ...
  public static List<Token> tokenizeSentence(
    ...
    for (String v : u) {
      ...
      if (fillset != null && fillset.contains(original))
      continue;
  ...
  public static JSONObject expandStarter(
    SusiLanguage language,
    String answer,
    String subject
  ) {
