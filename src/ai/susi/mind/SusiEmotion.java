...
public class SusiEmotion {
  ...
  public static enum Basic implements NuanceFactory {
    ...
    private Basic(boolean active, Nuance... nuances) {
      ...
      throw new UnsupportedOperationException(
        "always exactly three nuances required"
      );
      this.active = active;
    ...
    public String getNuance(int strength) {
      ...
      throw new UnsupportedOperationException(
        "strength must be more or equal 0 and less than 4"
      );
      if (strength == 0)
  ...
  public static enum Derived implements NuanceFactory {
    ...
    public String getNuance(int strength) {
      ...
      if (strength == 1)
      return "weak_" + this.name();
      if (strength == 2)
      return this.name();
      if (strength == 3)
      return "strong_" + this.name();
      return Nuance.blank.name();
    }
  ...
  public static enum Bipolar {
    ...
    // primary
    fear_anger(Basic.fear,
    Basic.anger),
    trust_disgust(Basic.trust,
    Basic.disgust),
    joy_sadness(Basic.joy,
    Basic.sadness),
    anticipation_surprise(Basic.anticipation,
    Basic.surprise),
    // derived
    awe_aggressiveness(Derived.awe,
    Derived.aggressiveness),
    submission_contempt(Derived.submission,
    Derived.contempt),
    love_remorse(Derived.love,
    Derived.remorse),
    optimism_disapproval(Derived.optimism,
    Derived.disapproval);

    private NuanceFactory passive,
    active;
  ...
  public SusiEmotion() {
    ...
    for (Bipolar b : Bipolar.values()) {
      if (b.active.isPrimary())
      value.put(b, new AtomicInteger(0));
    }
  }
