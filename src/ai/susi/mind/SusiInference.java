...
public class SusiInference {
  ...
  private static final ScriptEngine javascript = new ScriptEngineManager(
  ...
  /**
     ...
     */
  public Type getType() {
    return this.json.has("type") ? Type.valueOf(
  ...
  public JSONObject getDefinition() {
    return this.json.has("definition") ? this.json.getJSONObject(
      "definition"
    ) : null;
  }
  ...
  static {
    flowProcedures.put(
      Pattern.compile("SQUASH"),
      (flow, matcher) -> {// perform a full mindmeld
    ...
    memoryProcedures.put(
      Pattern.compile("SET\\h+?([^=]*?)\\h+?=\\h+?([^=]*)\\h*?"),
      (flow, matcher) -> {
        String remember = matcher.group(1),
        matching = matcher.group(2);
        return see(
          flow,
          flow.unify("%1% AS " + remember, false, 0),
          flow.unify(matching, false, 0),
          Pattern.compile("(.*)")
        );
      }
    );
    memoryProcedures.put(
      ...
      (flow, matcher) -> {
        ...
        return see(
          flow,
          flow.unify(remember, false, 0),
          flow.unify(matching, false, 0),
          Pattern.compile(flow.unify(pattern, false, 0))
        );
      }
    );
    memoryProcedures.put(
      Pattern.compile("CLEAR\\h+?(.*)\\h*?"),
      (flow, matcher) -> {
        String clear = matcher.group(1);
        return see(
          flow,
          "%1% AS " + flow.unify(clear, false, 0),
          "",
          Pattern.compile("(.*)")
        );
      }
    ...
    memoryProcedures.put(
      Pattern.compile("IF\\h+?([^=]*)\\h*?"),
      (flow, matcher) -> {
        String expect = matcher.group(1);
        SusiThought t = see(
          flow,
          "%1% AS EXPECTED",
          flow.unify(expect, false, 0),
          Pattern.compile("(.+)")
        );
    ...
    memoryProcedures.put(
      ...
      (flow, matcher) -> {
        ...
        SusiThought t = see(
          ...
          "%1% AS EXPECTED",
          flow.unify(expect, false, 0),
          Pattern.compile(flow.unify(matching, false, 0))
        );
        if (t.isFailed() || t.hasEmptyObservation("EXPECTED"))
        return new SusiThought(); // empty thought -> fail
        return t;
    ...
    memoryProcedures.put(
      Pattern.compile("NOT\\h+?([^=]*)\\h*?"),
      (flow, matcher) -> {
        String reject = matcher.group(1);
        SusiThought t = see(
          flow,
          "%1% AS EXPECTED",
          flow.unify(reject, false, 0),
          Pattern.compile("(.*)")
        );
        if (t.isFailed() || t.hasEmptyObservation("EXPECTED"))
  ...
  /**
     ...
     */
  private static final SusiThought see(
    SusiArgument flow,
    String transferExpr,
    String expr,
    Pattern pattern
  ...
  public SusiThought applyProcedures(SusiArgument flow) {
    ...
    if (type == SusiInference.Type.console) {
      String expression = this.getExpression(); // we have two ways to define a console rule:
      ...
      if (expression.length() == 0) {
        ...
        if (definition.has("url") && definition.has("path"))
        try {
          ...
          if (data != null) {
            ...
            if (learned.length() > 0 && definition.has(
              "actions"
            ) && learned.get(0) instanceof JSONObject && definition.get(
              "actions"
            ) instanceof JSONArray) {
              learned.getJSONObject(0).put(
                "actions",
                definition.getJSONArray("actions")
              );
            }
        ...
        }  else if (definition.has("actions") && definition.get(
          "actions"
        ) instanceof JSONArray) {
          ...
          JSONArray learned = new JSONArray().put(
            new JSONObject().put("actions", definition.getJSONArray("actions"))
          );
          json.setData(learned);
          json.setHits(json.getCount());
        }
        ...
        try {
          return ConsoleService.dbAccess.deduce(
            flow,
            flow.unify(expression, false)
          );
        } catch(Exception e) {}
      }
    }
