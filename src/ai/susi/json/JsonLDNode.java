...
public class JsonLDNode
  ...
  private String toRDFTriple(String subject, String vocabulary) {
    ...
    for (String predicate : getPredicates()) {
      ...
      sb.append(
        "<" + subject + "> <" + vocabulary + "#" + predicate + "> <" + (String) value + ">\n"
      );
      if (value instanceof JsonLDNode)
      sb.append(((JsonLDNode) value).toRDFTriple(subject, vocabulary));
    }
    return sb.toString();
  }
  ...
  /*
{
  ...
  public static void main(String[] args) {
    JsonLDNode event = new JsonLDNode("Event", "http://schema.org").setSubject(
      "http://an.event.home.page.ninja/tomorrow.html"
    ).setPredicate("name", "Typhoon with Radiation City").setPredicate(
      "startDate",
      "2013-09-14T21:30"
    ).setPredicate(
      "location",
      new JsonLDNode("Place").setPredicate("name", "The Hi-Dive").setPredicate(
        "address",
        new JsonLDNode("PostalAddress").setPredicate(
          "addressLocality",
          "Denver"
        ).setPredicate("addressRegion", "CO").setPredicate(
          "postalCode",
          "80209"
      ...
      "offers",
      new JsonLDNode("Offer").setPredicate("price", "13.00").setPredicate(
        "priceCurrency",
        "USD"
      ).setPredicate("postalCode", "80209").setPredicate(
        "url",
