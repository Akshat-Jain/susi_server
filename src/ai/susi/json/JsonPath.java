...
public class JsonPath {
  ...
  public static JSONArray parse(byte[] b, String jsonPath)
    throws
      JSONException {
    ...
    try {
      ...
      if (jsonPath.equals("$")) {
        return new JSONArray().put(
          new JSONObject().put("object", new String(b, StandardCharsets.UTF_8))
        );
      } else {
        throw e;
      }
  ...
  private static JSONArray parse(JSONTokener tokener, String jsonPath)
    throws
      JSONException {
    JSONArray a = parseRaw(tokener, jsonPath);
    if (a.length() == 0)
    return a; // length == 1 will cause an empty thought. Its not wrong, it will just cause that thinking fails. May be wanted.
    Object f = a.get(0);
    if (a.length(

    ) == 1 && (!(f instanceof JSONObject))) {// wrap this atom into an object with key name "object"
    ...
    for (int i = 1; i < a.length(); i++) {
      if (a.get(i).getClass() != c)
      throw new JSONException(
        "all objects in the result array must be of same type"
      );
    }
    ...
    throw new JSONException(
      "the objects in the result array must not be an array"
    );
  ...
  /**
     ...
     */
  private static JSONArray parseRaw(JSONTokener tokener, String jsonPath)
    throws
      JSONException {
    if (tokener == null)
    return null;
    String[] dompath = jsonPath.split("\\.");
    if (dompath == null || dompath.length < 1)
    return null; // wrong syntax of jsonPath
    if (dompath.length == 1) {
      if (dompath[0].equals("$"))
      return new JSONArray(tokener);
      if (dompath[0].length() > 1 && dompath[0].charAt(
        1
      ) == '[' && dompath[0].charAt(dompath[0].length() - 1) == ']') {
        ...
        for (int domc = 1; domc < dompath.length; domc++) {
            ...
            int p = path.indexOf('[') if (p < 0) {
        decomposition = ((decomposition == null) ? new JSONObject(
          tokener
        ) : ((JSONObject) decomposition)).get(path);
      } else if (p == 0) {
        int idx = Integer.parseInt(path.substring(1, path.length() - 1));
        decomposition = ((decomposition == null) ? new JSONArray(
          tokener
        ) : ((JSONArray) decomposition)).get(idx);
      } else {
    ...
    return (JSONArray) decomposition;
    if (decomposition instanceof JSONObject) {
      ...
      String[] columns = ((JSONObject) decomposition).keySet().toArray(
        new String[((JSONObject) decomposition).length()]
      );
      int rowcount = 0;
      for (String column : columns) {
        ((JSONObject) decomposition).put("k" + rowcount, column);
        ((JSONObject) decomposition).put(
          "v" + rowcount,
          ((JSONObject) decomposition).get(column)
        );
        rowcount++;
      }
    ...
    if (decomposition instanceof String)
    return new JSONArray().put(
      new JSONObject().put("object", (String) decomposition)
    );
    if (decomposition instanceof Integer)
    return new JSONArray().put(
      new JSONObject().put("object", decomposition.toString())
    );
    if (decomposition instanceof Double)
    return new JSONArray().put(
      new JSONObject().put("object", decomposition.toString())
    );
    throw new JSONException(
      "unrecognized object type: " + decomposition.getClass().getName()
    );
  }
  ...
  public static void main(String[] args) {
    ...
    test(
      "{\"query\":{\"count\":1,\"created\":\"2017-03-13T12:41:55Z\",\"lang\":\"de-DE\",\"results\":{\"row\":{\"symbol\":\"YHOO\",\"price\":\"45.98\",\"date\":\"3/10/2017\",\"time\":\"5:00pm\",\"change\":\"+0.04\",\"col1\":\"46.20\",\"high\":\"46.20\",\"low\":\"45.75\",\"col2\":\"3824629\"}}}}",
      "$.query.results.row"
    );
  }
