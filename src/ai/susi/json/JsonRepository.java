...
public class JsonRepository {
  ...
  public final static byte[][] META_KEYS = new byte[][] {
    OPERATION_KEY,
    MOD_DATE_KEY,
    REFERRER_KEY
  };
  ...
  private final static SimpleDateFormat dateFomatMonthly = new SimpleDateFormat(
    "yyyyMM",
    Locale.US
  );
  private final static SimpleDateFormat dateFomatDaily = new SimpleDateFormat(
    "yyyyMMdd",
    Locale.US
  );
  private final static SimpleDateFormat dateFomatHourly = new SimpleDateFormat(
    "yyyyMMddHH",
    Locale.US
  );
  private final static SimpleDateFormat dateFomatMinutely = new SimpleDateFormat(
    "yyyyMMddHHmm",
    Locale.US
  );
  ...
  public static enum Mode {
    ...
    COMPRESSED,
    // dump files are compressed but cannot be re-written. All data is cached in RAM.
    REWRITABLE;
  ...
  public JsonRepository(
    ...
    if (readme != null) {
      ...
      if (!message_dump_dir_readme.exists()) {
        BufferedWriter w = new BufferedWriter(
          new FileWriter(message_dump_dir_readme)
        );
        w.write(readme);
        w.close();
      }
    ...
    this.json_log = new JsonRandomAccessFile(
      getCurrentDump(dump_dir_own, this.dump_file_prefix, mode, dailyDump),
      this.concurrency
    );
  ...
  private static File getCurrentDump(
    ...
    if (existingDumps != null) {
      for (String d : existingDumps) {// first check if the file is the current file: we never compress that to enable a write to the end of the file
        ...
        if (mode == COMPRESSED_MODE) {// all files should be compressed to enable small file sizes, but contents must be in RAM after reading
        ...
        } else {// all files should be uncompressed to enable random-access mode
  ...
  public JsonFactory write(JSONObject json, char opkey) throws IOException {
    ...
    sb.append(
      '{').append('\"').append(OPERATION_KEY == null ? "" : new String(OPERATION_KEY, 0, OPERATION_KEY.length, StandardCharsets.UTF_8)).append('\"').append(':').append('\"').append(opkey).append('\"').append(','
    );
  ...
  public void buffer(Date created_at, Map<String, Object> map)
    throws
      IOException {
  ...
  private static SortedSet<File> getDumps(
    final File path,
    final String prefix,
    final String suffix,
    int count
    ...
    for (String s : list) {
      if (prefix == null || s.startsWith(
        prefix
      ) && (suffix == null || s.endsWith(suffix)))
