...
public class JsonTray {
  ...
  public JsonTray(File file_persistent, File file_volatile, int cachesize) {
    ...
    if (this.file_volatile != null && this.file_volatile.exists())
    try {
      JSONObject j = JsonFile.readJson(this.file_volatile);
      for (String key : j.keySet())
      this.vol.put(key, j.getJSONObject(key));
    } catch(IOException e) {
      e.printStackTrace();
    }
  }
