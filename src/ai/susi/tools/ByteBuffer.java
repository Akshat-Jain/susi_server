...
public final class ByteBuffer
  ...
  public boolean startsWith(final byte[] bs, final int start) {
    ...
    for (int i = 0; i < bs.length; i++) {
      if (this.buffer[this.offset + i + start] != bs[i])
      return false;
    }
    return true;
  }
  ...
  public ByteBuffer trim(final int start, final int len) {
    ...
    throw new IndexOutOfBoundsException(
      "trim: start + len > length; this.offset = " + this.offset + ", this.length = " + this.length + ", start = " + start + ", len = " + len
    );
