...
public class HttpClient {
  ...
  public static class Agent {
    ...
    // the minimum delay between two accesses

    public Agent(
      final String userAgent,
      final String[] robotIDs,
  ...
  private final static String[] browserAgents = new String[] {
    // fake browser user agents are NOT AVAILABLE IN P2P OPERATION, only on special customer configurations (commercial users demanded this, I personally think this is inadvisable)
  ...
  public final static Agent googleAgentAgent = new Agent(
    "Googlebot/2.1 (+http://www.google.com/bot.html)",
    new String[] { "Googlebot", "Googlebot-Mobile" },
    minimumGlobalDeltaInit / 2,
    defaultClientTimeout
  );
  public final static String yacyProxyAgentName = "YaCyProxy";
  public final static Agent yacyProxyAgent = new Agent(
    "yacy - this is a proxy access through YaCy from a browser, not a robot (the yacy bot user agent is 'yacybot')",
    new String[] { "yacy" },
  ...
  static {
    ...
    browserAgent = new Agent(
      browserAgents[random.nextInt(browserAgents.length)],
      new String[] { "Mozilla" },
  ...
  /**
     ...
     */
  public static final String yacySystem = System.getProperty(
    "os.arch",
    "no-os-arch"
  ) + " " + System.getProperty(
    "os.name",
  ...
  public static void generateYaCyBot(String addinfo) {
    ...
    yacyInternetCrawlerAgent = new Agent(
      agentString,
      new String[] { "yacybot" },
      minimumGlobalDeltaInit,
      defaultClientTimeout
    );
    yacyIntranetCrawlerAgent = new Agent(
      agentString,
      new String[] { "yacybot" },
      minimumLocalDeltaInit,
      defaultClientTimeout
    ); // must have the same userAgent String as the web crawler because this is also used for snippets
  ...
  public static void generateCustomBot(
    String name,
    String string,
    int minimumdelta,
    int clienttimeout
  ) {
    if (name.toLowerCase().indexOf("yacy") >= 0 || string.toLowerCase().indexOf(
      "yacy"
    ) >= 0)
    return; // don't allow 'yacy' in custom bot strings
    String agentString = string.replace(
      "$$SYSTEM$$",
      yacySystem.replace("java", "O")
    );
    agents.put(
      customAgentName,
      new Agent(agentString, new String[] { name }, minimumdelta, clienttimeout)
    );
  }
  ...
  public static String generateLocation() {
    ...
    if (lastSemicolon < firstClosedParenthesis) {
      // ; Location )
      location = (firstClosedParenthesis > 0) ? userAgent.substring(
        lastSemicolon + 1,
        firstClosedParenthesis
  ...
  ).setSocketTimeout(defaultClientTimeout).setConnectTimeout(
    defaultClientTimeout
  ).setConnectionRequestTimeout(
    defaultClientTimeout
  ).setContentCompressionEnabled(true).setCookieSpec(
  ...
  /**
     ...
     * @throws IOException
     */
  public HttpClient(String urlstring, boolean useAuthentication) {
    this.httpClient = HttpClients.custom().useSystemProperties(

    ).setConnectionManager(
      getConnctionManager(useAuthentication)
    ).setDefaultRequestConfig(defaultRequestConfig).build();
    this.request = new HttpGet(urlstring);
    this.request.setHeader(
      "User-Agent",
      getAgent(yacyInternetCrawlerAgentName).userAgent
    );
    this.init();
  }
  ...
  /**
     ...
     * @throws IOException
     */
  public HttpClient(String urlstring, Map<String, byte[]> map) {
  ...
  public static PoolingHttpClientConnectionManager getConnctionManager(
    ...
    PoolingHttpClientConnectionManager cm = socketFactoryRegistry != null ? new PoolingHttpClientConnectionManager(
  ...
  private void init() throws IOException {
    ...
    try {
      ...
      throw new IOException(
        "client connection failed: unknown host " + this.request.getURI(
      ...
      this.request.releaseConnection();
      throw new IOException(
        "client connection timeout for request: " + this.request.getURI()
      );
    } catch(SSLHandshakeException e) {
      this.request.releaseConnection();
      throw new IOException(
        "client connection handshake error for domain " + this.request.getURI(
    ...
    if (httpEntity != null) {
      if (this.httpResponse.getStatusLine().getStatusCode() == 200) {
        ...
        this.request.releaseConnection();
        throw new IOException(
          "client connection to " + this.request.getURI(

          ) + " fail: " + status + ": " + httpResponse.getStatusLine(
      ...
      this.request.releaseConnection();
      throw new IOException(
        "client connection to " + this.request.getURI() + " fail: no connection"
      );
    }
  }
  ...
  /**
     ...
     */
  public static String getRedirect(String urlstring, boolean useAuthentication)
    throws
      IOException {
    ...
    if (httpEntity != null) {
      if (httpResponse.getStatusLine().getStatusCode() == 301) {
        ...
        EntityUtils.consumeQuietly(httpEntity);
        throw new IOException(
          "redirect for  " + urlstring + ": no location attribute found"
        );
      } else {
        EntityUtils.consumeQuietly(httpEntity);
        throw new IOException(
          "no redirect for  " + urlstring + " fail: " + httpResponse.getStatusLine(
      ...
      throw new IOException(
        "client connection to " + urlstring + " fail: no connection"
      );
    }
  }
  ...
  /**
     ...
     * @throws IOException
     */
  public static byte[] load(String source_url, Map<String, byte[]> post)
    throws
      IOException {
  ...
  public static JSONArray loadJSONArray(String source_url) throws IOException {
    ...
    return new JSONArray(
      new JSONTokener(new String(b, StandardCharsets.UTF_8))
    );
  }
  ...
  public static JSONObject loadJSONObject(String source_url)
    throws
      IOException {
    byte[] b = load(source_url);
