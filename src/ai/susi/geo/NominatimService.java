...
public class NominatimService {
  ...
  public static class LocationNotFoundException
    extends Exception {
  ...
  public static IntegerGeoPoint integerGeoPoint(String locationName)
    throws
      URISyntaxException,
  ...
  public static ArrayList<IntegerGeoPoint> integerGeoPoints(
    String locationName,
    int limit
  )
    throws
    ...
    String rawJson = NominatimService.getRawJsonResponse(
      NominatimService.getUri(locationName)
    );
    ...
    if (places.length() == 0) {
      ...
      for (int i = 0; i < places.length(); i++) {
        ret.add(
          new IntegerGeoPoint(place.getDouble("lat"), place.getDouble("lon"))
        );
      }
    }
    return ret;
  ...
  private static URI getUri(String locationName) throws URISyntaxException {
    ...
    String provider = NominatimService.nominatimProviders.get(
      (new Random()).nextInt(NominatimService.nominatimProviders.size())
    );
  ...
  private static String getRawJsonResponse(URI uri)
    throws
      ClientProtocolException,
      IOException {
    HttpGet httpGet = new HttpGet(uri);

    CloseableHttpClient client = HttpClientBuilder.create().setUserAgent(
      NominatimService.userAgent
    ).build();
