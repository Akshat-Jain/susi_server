...
public class GeoJsonReader implements Runnable {
  ...
  public GeoJsonReader(final InputStream is, final int concurrency) {
    this.concurrency = concurrency;
    this.featureQueue = new ArrayBlockingQueue<Feature>(
  ...
  public void run(
    ...
    try {
      JsonToken token;
      while (!parser.isClosed() && token = parser.nextToken(

      ) != null && token != JsonToken.END_OBJECT) {
        ...
        if (JsonToken.FIELD_NAME.equals(token) && "features".equals(name)) {
          ...
          ) != null && token != JsonToken.END_ARRAY) {
  ...
  public static class Feature {
    ...
    // only used to create a POISON object
    public Feature(JsonParser parser) {
      ...
      while (!parser.isClosed() && token = parser.nextToken(
        ...
        if (JsonToken.FIELD_NAME.equals(token) && "type".equals(name)) {
          parser.nextToken();
          this.feature_type = parser.getText();
          continue;
        }
        ...
        if (JsonToken.FIELD_NAME.equals(token) && "geometry".equals(name)) {
          ...
          while (!parser.isClosed() && token = parser.nextToken(
            ...
            if (JsonToken.FIELD_NAME.equals(token) && "type".equals(name)) {
              parser.nextToken();
              this.geometry_type = parser.getText();
            } else if (JsonToken.FIELD_NAME.equals(
              token
    ...
    public String toString() {
      return "id:" + id + "; lon:" + properties.get(
        "geo_longitude"
      ) + "; lat:" + properties.get("geo_latitude");
    }

  }

  private static Map<String, String> parseMap(JsonParser parser)
    throws
      JsonParseException,
      IOException {
