...
public class OSMTile {
  ...
  /**
     ...
     * @return the image
     */
  public static RasterPlotter getCombinedTiles(
    final TileCoordinates t,
    int width,
    int height
  ) {
    ...
    for (int i = 0; i < width; i++) {
      for (int j = 0; j < height; j++) // be nice with tile server for uncached tiles
      {
        place = new Place(
          m,
          t.xtile - w + i,
  ...
  public static BufferedImage getSingleTile(
  ...
  public static class TileCoordinates {
    ...
    public TileCoordinates(
      final double lat_deg,
      final double lon_deg,
      final int zoom
    ...
    private double tile2lat(
      int y
    ) {//return Math.toDegrees(Math.atan(Math.sinh(Math.PI * (1 - 2 * y) / this.n)));

      return Math.toDegrees(
        Math.atan(Math.sinh(Math.PI - 2.0 * Math.PI * y / this.n))
      );
    }
