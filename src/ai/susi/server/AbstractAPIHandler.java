...
public abstract class AbstractAPIHandler
  extends HttpServlet implements APIHandler {
  ...
  public abstract ServiceResponse serviceImpl(
    Query post,
  ...
  private void process(
    ...
    if (minimalUserRole == null)
    minimalUserRole = UserRole.ANONYMOUS;
    ...
    if (DAO.getConfig(
      "users.admin.localonly",
      true
    ) && minimalUserRole == UserRole.ADMIN && !query.isLocalhostAccess()) {
    ...
    if (authorization.getUserRole().ordinal() < minimalUserRole.ordinal()) {
      String message = "Base user role not sufficient. Your base user role is '" + authorization.getUserRole(

      ).name() + "', your user role is '" + authorization.getUserRole().getName(
    ...
    try {
      ServiceResponse serviceResponse = serviceImpl(
        query,
        response,
        authorization,
        new JsonObjectWithDefault(authorization.getPermission())
      );
      ...
      if (serviceResponse.isObject() || serviceResponse.isArray()) {
        ...
        logClient(
          ...
          identity,
          200,
          "ok: " + (minified ? out : serviceResponse.toString(true))
        );
  ...
  /**
     ...
     */
  private ClientIdentity getIdentity(
    long startTime,
    HttpServletRequest request,
    HttpServletResponse response,
    Query query
    ...
    if (getLoginCookie(request) != null) {
      ...
      ) != null && authentication.checkExpireTime()) {
      ...
      DAO.log(
        "Invalid login try via cookie from host: " + query.getClientHost()
      );
    } else if (request.getSession().getAttribute("identity") != null) {
      // check session is set
      ClientIdentity identity = (ClientIdentity) request.getSession(

      ).getAttribute("identity");
      DAO.log("USER REQUEST using browser session: " + identity.getClient());
      return identity;
    } else if (request.getParameter("access_token") != null) {
      // access tokens can be used by api calls, somehow the stateless equivalent of sessions for browsers
      ClientCredential credential = new ClientCredential(
        ...
        if (authentication.checkExpireTime()) {
          ...
          if (authentication.has("one_time") && authentication.getBoolean(
            ...
            logClient(
              ...
              identity,
              0,
              "user requests a one-time session, authentication deleted"
            );
          }
          return identity;
        }
  ...
  public void logClient(
    ...
    DAO.log(
      host + " - " + username + " - " + httpResponseCode + " - " + t + "ms - " + path + " - " + message
    );
  }

  public static String getRequestHeaderSalt(HttpServletRequest request) {
    String idhint = request.getHeader("User-Agent") + request.getHeader(
      "Accept-Encoding"
  ...
  /**
     ...
     */
  private static ClientIdentity getAnonymousIdentity(
    String remoteHost,
    String salt
  ) {
    ClientCredential credential = new ClientCredential(
      ClientCredential.Type.host,
      remoteHost + "_" + salt
    );
    Authentication authentication = DAO.getAuthentication(credential);

    if (authentication.getIdentity() == null)
    authentication.setIdentity(credential);
    authentication.setExpireTime(
      Instant.now().getEpochSecond() + defaultAnonymousTime
    );
