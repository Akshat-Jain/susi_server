...
public class PublicKeyRegistrationService
  ...
  public ServiceResponse serviceImpl(
    ...
    throws
      APIException {
    if (post.get("register", null) == null && !post.get(
      "create",
      false
    ) && !post.get("getParameters", false)) {
      throw new APIException(
        400,
        "Accepted parameters: 'register', 'create' or 'getParameters'"
      );
    }
    ...
    id = post.get("id", null); else
    id = authorization.getIdentity().getName();
    // check if we are allowed register a key

    if (!id.equals(
      authorization.getIdentity().getName()
    )) {// if we don't want to register the key for the current user
      ...
      if (permissions.getJSONObject("users", null).has(
        id
      ) && permissions.getJSONObjectWithDefault("users", null).getBoolean(
        id,
        false
      )) {
        allowed = true;
        ...
        // check if the user role of the user in question is in 'userRoles'
      ...
      throw new APIException(
        403,
        "You are not allowed to register a public key"
      );
    }
    ...
    if (post.get("algorithm", null) != null) {
      ...
      "create",
      false
    )) {// create a new key pair on the server

      if (algorithm.equals("RSA")) {
        int keySize = 2048;
        if (post.get("key-size", null) != null) {
          int finalKeyLength = post.get("key-size", 0);
          if (!IntStream.of(allowedKeySizesRSA).anyMatch(
            x -> x == finalKeyLength
          )) {
        ...
        result.put(
          "publickey_DER_BASE64",
          Base64.getEncoder().encodeToString(keyPair.getPublic().getEncoded())
        );
        result.put(
          "privatekey_DER_BASE64",
        ...
        try {
          result.put(
            "keyhash_urlsave",
            URLEncoder.encode(IO.getKeyHash(keyPair.getPublic()), "UTF-8")
          );
      ...
      if (algorithm.equals("RSA")) {
        ...
        if (type.equals("DER")) {
          try {
            ...
            pub = (RSAPublicKey) KeyFactory.getInstance(
              algorithm
        ...
        try {
          result.put(
            "keyhash_urlsave",
            URLEncoder.encode(IO.getKeyHash(pub), "UTF-8")
          );
        } catch(UnsupportedEncodingException e) {}
