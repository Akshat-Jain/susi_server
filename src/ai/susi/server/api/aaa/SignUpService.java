...
public class SignUpService
  ...
  public ServiceResponse serviceImpl(
    ...
    if (post.get("getParameters", false)) {
      ...
      String passwordPatternTooltip = DAO.getConfig(
        "users.password.regex.tooltip",
    ...
    if (post.get("validateEmail", null) != null) {
      if (auth.getIdentity().getName().equals(
        post.get("validateEmail", null)
      ) && auth.getIdentity().isEmail(

      ) // the user is logged in via an access token from the email
      || permissions.getBoolean(
        "activate",
        false
      )) {// the user is allowed to activate other users
    ...
    boolean activated;
    boolean sendEmail;
    if (permissions.getBoolean("register", false)) {
      // if this registration is done by user that is allowed to register new users
      activated = true;
      sendEmail = false;
    } else {
      switch (DAO.getConfig("users.public.signup", "false")) {
        ...
        default:
          throw new APIException(
            500,
            "Invalid value for key: users.public.signup"
          );
      }
    }
    if (post.get("signup", null) == null || post.get(
      "password",
      null
    ) == null) {
    ...
    if (signup.equals(password) || !new TimeoutMatcher(
    ...
    if (sendEmail) {
      ...
      try {
        ...
        result.put(
          "message",
          "You successfully signed-up, but no email was sent: " + e.getMessage()
        );
      }
    } else {
  ...
  private String getVerificationMailContent(String token, String userId)
    ...
    try {
      result = IO.readFileCached(
        Paths.get(DAO.conf_dir + "/templates/verification-mail.txt")
      );
    } catch(IOException e) {
