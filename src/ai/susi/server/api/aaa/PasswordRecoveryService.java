  ...
  public ServiceResponse serviceImpl(
    ...
    if (call.get("getParameters", false)) {
      if (call.get("token", null) != null && !call.get("token", null).isEmpty(
        ...
        ClientCredential credentialcheck = new ClientCredential(
        ...
        if (DAO.passwordreset.has(credentialcheck.toString())) {
          ...
          if (authentication.checkExpireTime()) {
            ...
            result.put(
              "message",
              "Email ID: " + authentication.getIdentity().getName()
            );
    ...
    ClientCredential tokenkey = new ClientCredential(
      ClientCredential.Type.resetpass_token,
      token
    );
    ...
    try {
      EmailHandler.sendEmail(
        usermail,
        subject,
        getVerificationMailContent(token)
      );
      result.put("accepted", true);
      result.put(
        "message",
        "Recovery email sent to your email ID. Please check"
      );
    } catch(Exception e) {
  ...
  private String getVerificationMailContent(String token) {
    ...
    try {
      result = IO.readFileCached(
        Paths.get(DAO.conf_dir + "/templates/reset-mail.txt")
      );
    } catch(IOException e) {
      result = "";
    }
    result = result.contains(resetLinkPlaceholder) ? result.replace(
