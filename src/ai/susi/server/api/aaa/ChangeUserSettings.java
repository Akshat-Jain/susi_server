  ...
  public ServiceResponse serviceImpl(
    ...
    if (count == -1) {
      throw new APIException(
        400,
        "Bad Service call, count parameters not provided"
      );
    } else {
      ...
      for (int i = 1; i <= count; i++) {
        ...
        if (key == null || value == null) {
          throw new APIException(
            400,
            "Bad Service call, key or value parameters not provided"
          );
        } else {
      ...
      if (authorization.getIdentity() == null) {
        throw new APIException(
          400,
          "Specified User Setting not found, ensure you are logged in"
        );
      } else {
        ...
        result.put(
          "message",
          "You successfully changed settings of your account!"
        );
