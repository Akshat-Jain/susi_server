...
// 100 MB
public class ModifySkillService
  ...
  @Override
  protected void doPost(HttpServletRequest call, HttpServletResponse resp)
    throws
      ServletException,
      IOException {
    // debug the call
    ...
    if (call.getParameter("access_token") != null) {
      ...
      File skill = SusiSkill.getSkillFileInLanguage(
        language,
        skill_name,
        false
      );
      ...
      File modified_skill = new File(
        modified_language,
        modified_skill_name + ".txt"
      );
      ...
      if (skill.exists() && content != null) {
        JSONObject json = new JSONObject(); // IF SKILL AND IMAGE PATH IS CHANGED
        // CHECK IF SKILL PATH AND NAME IS SAME. IF IT IS SAME THEN MAKE CHANGES IN OLD FILE ONLY

        if (model_name.equals(modified_model_name) && group_name.equals(
          modified_group_name
        ) && language_name.equals(modified_language_name) && skill_name.equals(
          modified_skill_name
        )) {// Writing to File
          ...
          if (image_changed.equals("true")) {
            ...
            if (file != null) {
              ...
              Path new_path = Paths.get(
                language + File.separator + "images/" + new_image_name
              );
              Path old_path = Paths.get(
                language + File.separator + "images/" + old_image_name
              );
              ...
              BufferedImage bi = CreateSkillService.createResizedCopy(
                image,
                512,
                512,
                true
              ...
              if (!Files.exists(
                Paths.get(language.getPath() + File.separator + "images")
              )) {
              ...
              ImageIO.write(
                ...
                new File(
                  ...
                  ) + File.separator + "images/" + new_image_name
                )
              );
              json.put("message", "Skill updated");
              json.put("accepted", true);
          ...
          if (image_name_changed.equals("true") && image_changed.equals(
            "false"
          )) {
            String new_image_name = call.getParameter("new_image_name");
            String old_image_name = call.getParameter("old_image_name");
          ...
          String new_image_name = call.getParameter("new_image_name");
          Path new_path = Paths.get(
            modified_language + File.separator + "images/" + new_image_name
          );
          if (!Files.exists(
            Paths.get(modified_language.getPath() + File.separator + "images")
          )) {
            new File(
              modified_language.getPath() + File.separator + "images"
            ).mkdirs();
          }// write new file here
          ...
          if (!modified_skill.exists() && !Files.exists(new_path)) {
            ...
            if (image_changed.equals("true")) {
              ...
              if (file != null) {
                ...
                if (!Files.exists(new_path)) {
                  ...
                  json.put(
                    "message",
                    "The Image name not given or Image with same name is already present "
                  );
                }
              }
            } else {
            ...
            if (image_name_changed.equals("true") && image_changed.equals(
              ...
              String old_image_name = call.getParameter("old_image_name");
              Path old_path = Paths.get(
                modified_language + File.separator + "images/" + old_image_name
              );
              if (!Files.exists(new_path)) {
                old_path.toFile().renameTo(new_path.toFile());
                json.put("message", "Skill updated");
      ...
      if (call.getParameter("access_token") != null) {
        ...
        ClientCredential credential = new ClientCredential(
          ...
          call.getParameter("access_token")
        );
