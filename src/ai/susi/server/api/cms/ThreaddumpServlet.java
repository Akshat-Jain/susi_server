...
public class ThreaddumpServlet
  ...
  private static final Pattern multiDumpFilterPattern = Pattern.compile(
    multiDumpFilter
  );
  ...
  protected void doGet(
    ...
    if (servlet.length() > 0) {
      try {
        final Class<?> servletClass = ClassLoader.getSystemClassLoader(

        ).loadClass("org.loklak.api.server." + servlet);
        final Method getMethod = servletClass.getDeclaredMethod(
          "doGet",
    ...
    int multi = post.isLocalhostAccess() ? post.get(
      "multi",
      post.get("count", 0)
    ) : 0;
    ...
    try {
      ...
      if (method.getName().startsWith("get") && Modifier.isPublic(
        method.getModifiers()
      )) {
    ...
    if (multi > 0) {
      ...
      for (int i = 0; i < multi; i++) {
        try {
          ThreadDump dump = new ThreadDump(
            ThreadDump.getAllStackTraces(),
            Thread.State.RUNNABLE
          );
          for (final Map.Entry<StackTrace, SortedSet<String>> e : dump.entrySet(

          )) {
            if (new TimeoutMatcher(
              multiDumpFilterPattern.matcher(e.getKey().text)
            ).matches())
            continue;
            ...
            dumps.put(
              e.getKey().text,
              Integer.valueOf(e.getValue().size())
            ); else {
      ...
      final Map<
        Thread,
        StackTraceElement[]
      > stackTraces = ThreadDump.getAllStackTraces();
    ...
    bufferappend(
      buffer,
      "THREAD LIST FROM ThreadMXBean, " + threadbean.getThreadCount(
  ...
  private static class StackTrace {
    ...
    public boolean equals(final Object a) {
      return (a != null && a instanceof StackTrace && this.text.equals(
        ((StackTrace) a).text
      ));
    }
  ...
  private static Map.Entry<String, Integer> removeMax(
  ...
  private static void bufferappend(
    ...
    bufferappend(buffer, keylen, key, ((Number) value).longValue()); else
  ...
  private static class ThreadDump
    extends HashMap<StackTrace, SortedSet<String>>
    implements
      Map<StackTrace, SortedSet<String>> {
    ...
    private ThreadDump(
      ...
      > entry : stackTraces.entrySet()) {
        ...
        if (threadState != null && info != null && stateIn == null || stateIn.equals(
          threadState
        ) && stackTraceElements.length > 0) {
          final StringBuilder sb = new StringBuilder(3000);
          final String threadtitle = tracename + "THREAD: " + thread.getName(

          ) + " " + thread.isDaemon() ? "daemon" : "" + " id=" + thread.getId(

          ) + " " + threadState.toString() + (info.getLockOwnerId(

          ) >= 0 ? " lock owner =" + info.getLockOwnerId() : "");
          boolean cutcore = true;
          for (int i = 0; i < stackTraceElements.length; i++) {
            ste = stackTraceElements[i];
            String className = ste.getClassName();
            String classString = ste.toString();
            if (cutcore && (className.startsWith(
              "java."
