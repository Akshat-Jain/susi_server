...
public class ListSkillService
  ...
  public ServiceResponse serviceImpl(
    ...
    if (countString != null) {
      if (Integer.parseInt(countString) < 0) {
        throw new APIException(
          422,
          "Invalid count value. It should be positive."
        );
      } else {
        countFilter = true;
        try {
    ...
    if (group_name.equals("All")) {
      ...
      for (String temp_group_name : folderList) {
        ...
        for (String skill_name : fileList) {
          ...
          JSONObject skillMetadata = SusiSkill.getSkillMetadata(
            model_name,
            temp_group_name,
            language_name,
            skill_name,
    ...
    if (call.get("applyFilter", false)) {
          ...
          Collections.sort(
            ...
            new Comparator<JSONObject>() {
              ...
              public int compare(JSONObject a, JSONObject b) {
                ...
                try {
                  valA = a.getJSONObject("skill_rating").getJSONObject(
                    "stars"
                  ).getFloat("avg_star");
                  valB = b.getJSONObject("skill_rating").getJSONObject(
                    "stars"
                  ).getFloat("avg_star");
          ...
          Collections.sort(
            ...
            new Comparator<JSONObject>() {
              ...
              public int compare(JSONObject a, JSONObject b) {
                ...
                try {
                  valA = a.getJSONObject("skill_rating").getInt(
                    "feedback_count"
                  );
                  valB = b.getJSONObject("skill_rating").getInt(
                    "feedback_count"
                  );
                  result = Integer.compare(valA, valB);
                } catch(JSONException e) {
      ...
      for (int i = 0; i < jsonArray.length(); i++) {
        ...
        if (dateFilter) {
          ...
          String startDate = new Timestamp(timestamp).toString().substring(
            0,
            10
          ); //substring is used for getting timestamp upto date only
          String skillCreationDate = jsonValues.get(i).get(
            "creationTime"
        ...
        if (searchFilter) {
          ...
          String skillName = skillMetadata.get("skill_name").toString(

          ).toLowerCase();
          String authorName = skillMetadata.get("author").toString(

          ).toLowerCase();
          String skillDescription = skillMetadata.get("descriptions").toString(

          ).toLowerCase();
          Boolean skillMatches = false;
          if (!skillName.matches(searchQuery) && !authorName.matches(
            searchQuery
          ) && !skillDescription.matches(searchQuery)) {
            try {
              String skillExamples = skillMetadata.get("examples").toString(
    ...
    json.put("model", model_name).put("group", group_name).put(
      "language",
      language_name
    );
  ...
  private void listFilesForFolder(
    ...
    if (filesInFolder != null) {
      Arrays.stream(filesInFolder).filter(
        fileEntry -> !fileEntry.isDirectory() && !fileEntry.getName(
  ...
  private void listFoldersForFolder(
    final File folder,
    ArrayList<String> fileList
  ) {
    File[] filesInFolder = folder.listFiles();
    if (filesInFolder != null) {
      Arrays.stream(filesInFolder).filter(
        fileEntry -> fileEntry.isDirectory() && !fileEntry.getName().startsWith(
          "."
        )
      ).forEach(fileEntry -> fileList.add(fileEntry.getName() + ""));
    }
