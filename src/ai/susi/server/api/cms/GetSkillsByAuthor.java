 ...
 */
public class GetSkillsByAuthor
  ...
  public ServiceResponse serviceImpl(
    ...
    if (author_email == null) {
      for (Map.Entry<SusiSkill.ID, SusiSkill> entry : DAO.susi.getSkillMetadata(

      ).entrySet()) {
        SusiSkill.ID skill = entry.getKey();
        if (entry.getValue().getAuthor() != null && entry.getValue().getAuthor(
      ...
      result.put(
        "message",
        "Successfully fetched all Skills created by author."
      );
      return new ServiceResponse(result);
    }
  }
