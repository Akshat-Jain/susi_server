 ...
 */
public class FiveStarRateSkillService
  ...
  public ServiceResponse serviceImpl(
    ...
    if (!authorization.getIdentity().isAnonymous()) {
          ...
          if (groupName.has(language_name)) {
            ...
            if (languageName.has(skill_name)) {
              ...
              for (int i = 0; i < skillName.length(); i++) {
                ...
                if (authorization.getIdentity().isEmail() && ratingObject.get(
                  "email"
                ).equals(idvalue) || (authorization.getIdentity().isUuid(

                ) && ratingObject.get("uuid").equals(idvalue))) {
      ...
      if (!alreadyByUser) {
        ...
        ratingObject.put("email", idvalue);
        if (authorization.getIdentity().isUuid())
        ratingObject.put("uuid", idvalue);
        ratingObject.put("stars", skill_stars);
        ratingObject.put("timestamp", timestamp.toString());
  ...
  public JSONObject createRatingObject(String skill_stars) {
    ...
    float totalStars = skillStars.getInt("one_star") + skillStars.getInt(
      "two_star"
    ) + skillStars.getInt("three_star") + skillStars.getInt(
      "four_star"
    ) + skillStars.getInt("five_star");
    float avgStar = 1 * skillStars.getInt("one_star") + 2 * skillStars.getInt(
      "two_star"
    ) + 3 * skillStars.getInt("three_star") + 4 * skillStars.getInt(
      "four_star"
    ) + 5 * skillStars.getInt("five_star") / totalStars;
      ...
      if (modelName.has(group_name)) {
        ...
        if (groupName.has(language_name)) {
          ...
          if (languageName.has(skill_name)) {
            ...
            float totalStars = skillStars.getInt(
              "one_star"
            ) + skillStars.getInt("two_star") + skillStars.getInt(
              "three_star"
            ) + skillStars.getInt("four_star") + skillStars.getInt("five_star");
            float avgStar = 1 * skillStars.getInt(
              "one_star"
            ) + 2 * skillStars.getInt("two_star") + 3 * skillStars.getInt(
              "three_star"
            ) + 4 * skillStars.getInt("four_star") + 5 * skillStars.getInt(
              "five_star"
            ) / totalStars;
          ...
          if (languageName.has(skill_name)) {
            ...
            if (skillName.has("stars")) {
              ...
              if (skill_stars.equals("1")) {
                ...
                skillStars.put(
                  "three_star",
                  skillStars.getInt("three_star") + 1
                );
              } else if (skill_stars.equals("4")) {
                skillStars.put("four_star", skillStars.getInt("four_star") + 1);
              } else if (skill_stars.equals("5")) {
              ...
              float totalStars = skillStars.getInt(
                ...
                "three_star"
              ) + skillStars.getInt("four_star") + skillStars.getInt(
                "five_star"
              );
              float avgStar = 1 * skillStars.getInt(
                "one_star"
