...
public class SusiService
  ...
  public ServiceResponse serviceImpl(
    ...
    String persona = post.get("persona", "");
    // an instant dream setting, to be used for permanent dreaming
    ...
    if (recall != null && (dream == null || dream.length() == 0)) {
      ...
      try {
        ...
        JSONTokener serviceResponse = new JSONTokener(
          new ByteArrayInputStream(ConsoleService.loadData(padurl, dream))
        );
        ...
        SusiMind dreamMind = new SusiMind(
          DAO.susi_chatlog_dir,
          DAO.susi_skilllog_dir
        ); // we need the memory directory here to get a share on the memory of previous dialoges, otherwise we cannot test call-back questions
        JSONObject rules = SusiSkill.readLoTSkill(
          new BufferedReader(
    ...
    if (recall != null && (persona == null || persona.length() == 0)) {
      ...
      File skillfile = persona == null || persona.length(

      ) == 0 ? null : SusiSkill.getSkillFileInModel(
        new File(DAO.model_watch_dir, "persona"),
        persona
      );
      // read the persona
      ...
      try {
        String text = new String(
          Files.readAllBytes(skillfile.toPath()),
          StandardCharsets.UTF_8
        );
        ...
        // fill an empty mind with the dream
        SusiMind awakeMind = new SusiMind(
          DAO.susi_chatlog_dir,
          DAO.susi_skilllog_dir
        ); // we need the memory directory here to get a share on the memory of previous dialoges, otherwise we cannot test call-back questions
        JSONObject rules = SusiSkill.readLoTSkill(
    ...
    SusiCognition cognition = new SusiCognition(
      q,
      timezoneOffset,
      latitude,
      longitude,
      ...
      count,
      user.getIdentity(),
      minds.toArray(new SusiMind[minds.size()])
    );
    if (cognition.getAnswers().size() > 0)
    try {
      DAO.susi.getMemories().addCognition(
        user.getIdentity().getClient(),
        cognition
      );
    } catch(IOException e) {
