...
public class FileHandler
  extends ResourceHandler implements Handler {
  ...
  @Override
  protected void doResponseHeaders(
    HttpServletResponse response,
    Resource resource,
    String mimeType
  ) {
  ...
  public static void setCaching(
    ...
    if (response instanceof org.eclipse.jetty.server.Response) {
      ...
      fields.remove(HttpHeader.LAST_MODIFIED);
      // cache-control: allow shared caching (i.e. proxies) and set expires age for cache

      if (expiresSeconds == 0) {
        fields.put(
          HttpHeader.CACHE_CONTROL,
          "public, no-store, max-age=" + Integer.toString(expiresSeconds)
        ); // seconds
      } else {
        fields.put(
          HttpHeader.CACHE_CONTROL,
          "public, max-age=" + Integer.toString(expiresSeconds)
        ); // seconds
      }
    } else {
  ...
  public Resource getResource(String path) {
    ...
    try {
      ...
      if (cache != null)
      return cache;
      if (f.length() < CACHE_LIMIT || f.getName().endsWith(
        ".html"
      ) || path.equals("/")) {
  ...
  private static class CacheResource
    ...
    public CacheResource(PathResource pathResource) {
      this.file = pathResource.getFile();
      if (this.file.isDirectory())
      this.file = new File(this.file, "index.html");
    ...
    private void initCache(long nextLastModified) throws IOException {
      ...
      this.buffer = insertSSI(this.buffer);
    ...
    private long actualLastModified() {
      ...
      for (File d : this.includes)
      l = Math.max(l, d.lastModified());
      return l;
    }
    ...
    @Override
    public Resource addPath(String arg0)
      throws
        IOException,
