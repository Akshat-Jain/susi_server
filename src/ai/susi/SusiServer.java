...
public class SusiServer {
  ...
  public static ContextHandler buildSwaggerUI() throws Exception {
    ...
    rh.setResourceBase(
      SusiServer.class.getClassLoader().getResource(
  ...
  public static Map<String, String> readConfig(Path data) throws IOException {
    ...
    File customized_config = new File(
      settings_dir.toFile(),
  ...
  public static int getServerThreads() {
    return server.getThreadPool().getThreads() - server.getThreadPool(
  ...
  public static void main(String[] args) throws Exception {
    ...
    // no awt used here so we can switch off that stuff
    ...
    if (!dataFile.exists())
    dataFile.mkdirs();
    // should already be there since the start.sh script creates it
    ...
    if (pid.exists())
    pid.deleteOnExit();
    // thats a signal for the stop.sh script that SUSI has terminated
  ...
  private static void setupHttpServer(int httpPort, int httpsPort)
    throws
      Exception {
    ...
    if (!httpsMode.equals(HttpsMode.ONLY)) {
      ...
      if (httpsMode.equals(HttpsMode.REDIRECT)) {
        //redirect
    ...
    if (httpsMode.isGreaterOrEqualTo(HttpsMode.ON)) {
      ...
      if ("keystore".equals(keySource)) {
        ...
        File keystoreFile = new File(
          DAO.conf_dir,
          DAO.getConfig("keystore.name", "keystore.jks")
        );
        ...
        keyStore.load(
          new FileInputStream(keystoreFile.getAbsolutePath()),
        ...
        PEMParser parser = new PEMParser(
          new InputStreamReader(new ByteArrayInputStream(certBytes))
        );
        ...
        parser = new PEMParser(
          new InputStreamReader(new ByteArrayInputStream(keyBytes))
        );
        PrivateKey key = new JcaPEMKeyConverter().setProvider(
          "BC"
  ...
  private static void setServerHandler(File dataFile) {
    ...
    if (redirect || auth) {
      ...
      if (auth)
      SusiServer.server.addBean(loginService);
      ...
      if (redirect)
      DAO.log("Activated http-to-https redirect");
      if (auth)
      DAO.log("Activated basic http auth");
    }
    ...
    try {
      servletHandler.addServlet(
        service,
        ((APIHandler) (service.newInstance())).getAPIPath()
      );
    ...
    servletHandler.setMaxFormContentSize(10 * 1024 * 1024);
    // 10 MB
    ...
    FileHandler fileHandler = new FileHandler(
      Integer.parseInt(DAO.getConfig("www.expires", "600"))
    );
    ...
    handlerlist2.setHandlers(
      new Handler[] { fileHandler, rewriteHandler, new DefaultHandler() }
    );
    ...
    resourceConfig.packages(
      SusiServer.class.getPackage().getName(),
  ...
  private static void checkServerPorts(int httpPort, int httpsPort)
    throws
      IOException {// check http port
  ...
  private static void checkPort(int port, ServerSocket ss) throws IOException {
    ...
    } catch(IOException e) {// the socket is already occupied by another service

      throw new IOException(
        "port " + port + " is already occupied by another service, maybe another SUSI is running on this port already. exit."
      );
    } finally {// close the socket again
